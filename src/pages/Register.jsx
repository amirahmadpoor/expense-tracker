...
const Register = () => {
  ...
  const validate = () => {
    if (!fullName.trim() || fullName.length < 3) {
      toast.error('نام کامل حداقل 3 حرف');
      return false;
    }
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
    ...
    if (password.length < 6) {
      toast.error('رمز عبور حداقل 6 حرف');
      return false;
    }
    ...
    if (!agreeTerms) {
      toast.error('لطفا شرایط را قبول کنید');
      return false;
    }
    return true;
  };
  ...
  const strength = password.length >= 8 ? 'قوی' : password.length >= 6 ? 'متوسط' : '';
        ...
        <div className="card rounded-2xl p-8 md:p-10">
          ...
          <form onSubmit={handleRegister} className="space-y-4">
            ...
            <div>
              ...
              {strength && <p className={`text-xs mt-1 ${strength === 'قوی' ? 'text-success' : 'text-warning'}`}>سطح رمز: {strength}</p>}
            </div>
            ...
            <label className="flex items-start gap-2 cursor-pointer pt-2">
              <input
                ...
                className="w-4 h-4 mt-1 rounded border-2 border-border bg-surface checked:bg-success cursor-pointer accent-success flex-shrink-0"
                disabled={loading} />

              <span className="text-xs text-text-secondary leading-5">من <a href="#" className="text-success font-medium">شرایط</a> و <a href="#" className="text-success font-medium">حریم خصوصی</a> را قبول می‌کنم</span>
            </label>
